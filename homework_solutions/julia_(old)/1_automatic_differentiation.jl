...
# ╔═╡ 1e754fca-51aa-11f0-3e59-db1079a6a142
# ╠═╡ disabled = true
#=╠═╡
begin
	import Pkg
...
# ╔═╡ 7918e600-2587-4eb8-b379-ee62c7960298
Fˣ(x,y) = 3*x^2 + cos(x)*y^2

# ╔═╡ 54943ca2-f6cc-44fe-b8df-c16053117802
Fʸ(x,y) = sin(x)*2*y + y^-1
...
# ╔═╡ 3bf21d48-b8c3-4776-95df-51ac51dc1dac
function signal_plus_background(x,pars) #Exponential + Background function
	μ,σ,τ,a = pars[1],pars[2],pars[3],pars[4]
	signal = ((2*pi*σ*σ)^(-0.5)) * exp(-((x-μ)^2)/(2*σ*σ))
	background = τ*exp(-τ*x)
	return a*signal + (1-a)*background
end
...
# ╔═╡ d6fb7d71-9579-4419-8e1f-3f24482fce93
...
function ReturnOptimParsWithTime(cost,init_pars;derivative=nothing)
	#cost(init_pars) = nll(signal_plus_background,init_pars,DataSample[1:n_sample])
	t= @elapsed begin
		m=Minuit(cost,init_pars;grad=derivative)
		migrad!(m)
	end
	return m,t
end
...
let
	mt1=Minuit(objective,[1.0,0.4,10.0,0.9]) #Defines a minuit type object
	migrad!(mt1)
	#TODO Get fit parameters and compare with your starting parameters of the function
end
...
function forward_gradient_function(pars) #Has to be a local function dependent on "objective".
	p_julia=Float64[]
	for i in pars
		push!(p_julia,i)
	end
    return ForwardDiff.gradient(objective, p_julia)
end

# ╔═╡ 3f15183c-c928-4df5-bc1c-0087c85b8b78
#TODO Use your function ReturnOptimParsWithTime without derivative
m_noderivative,t_noderivative = ReturnOptimParsWithTime(objective,[1.0,0.4,10.0,0.9])
...
m_withderivative,t_withderivative = ReturnOptimParsWithTime(objective,[1.0,0.4,10.0,0.9];derivative=forward_gradient_function)
...
let
    ...
    dx = bins[2] - bins[1]
    n = length(DataSample)
	scale = dx*n  #Scaling the function to take into account bin widths
	xlabel!("x")
	ylabel!("y")
	plot!(x->scale*signal_plus_background(x,m_noderivative.values), x_range..., label="Minuit without derivative")
	plot!(x->scale*signal_plus_background(x,m_withderivative.values), x_range..., label="Minuit with derivative")
end

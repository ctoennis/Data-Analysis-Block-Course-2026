...
@app.cell
def _(f2_1D, n_sample, np, pd, train_fraction):
    # Solution
    x_1d = 2 * np.pi * np.random.rand(n_sample)
    y_1d = f2_1D(x_1d) + 0.01 * np.random.standard_normal(n_sample)

    data_1d = pd.DataFrame({"x_data": x_1d, "y_data": y_1d})
    data_1d = data_1d.sample(frac=1.0, random_state=123).reset_index(drop=True)
...
@app.cell
def _(f2_2D, n_sample, np, pd, train_fraction):
    # Solution
    x_2d = 2 * np.pi * np.random.rand(n_sample) + 1.0
    y_2d = 2 * np.pi * np.random.rand(n_sample) + 1.0
    z_2d = f2_2D(x_2d, y_2d) + 0.005 * np.random.standard_normal(n_sample)

    data_2d = pd.DataFrame({"x_data": x_2d, "y_data": y_2d, "z_data": z_2d})
    data_2d = data_2d.sample(frac=1.0, random_state=123).reset_index(drop=True)

    n_train_2d = int(np.floor(n_sample * train_fraction))
    return data_2d, n_train_2d
...
def _(f2_2D, np, plt):
    ...
    surface_axis = np.linspace(1.0, 1.0 + 2 * np.pi, 100)
    surface_x, surface_y = np.meshgrid(surface_axis, surface_axis)
    surface_z = f2_2D(surface_x, surface_y)
...
def _(DecisionTreeRegressor, data_2d, n_train_2d, tree_depth):
    ...
    return ex2d_test, features_2d, model_2d, ztrue_2d
...
@app.cell
def _(ex2d_test, model_2d, np, plt, surface_x, surface_y, surface_z):
    # Solution
    test_features_2d = ex2d_test[["x_data", "y_data"]].to_numpy()
    z_pred_tree_test = model_2d.predict(test_features_2d)
    test_mse_2d = np.mean((z_pred_tree_test - ex2d_test["z_data"].to_numpy()) ** 2)
    ...
    ax_2d_test.scatter(ex2d_test["x_data"], ex2d_test["y_data"], ex2d_test["z_data"], label="test data")
    ax_2d_test.scatter(ex2d_test["x_data"], ex2d_test["y_data"], z_pred_tree_test, label="single tree")

    ax_2d_test.set_xlabel("x")
    ax_2d_test.set_ylabel("y")
    ax_2d_test.set_zlabel("F(x, y)")
    ax_2d_test.set_title("2D regression tree: test data vs prediction")
    ax_2d_test.legend()
    return
...
def _(features_1d, n_trees, tree_depth, ytrue_1d):
    ...
    return RandomForestRegressor, forest_1d
...
@app.cell
def _(f2_1D, forest_1d, model_1d, n_trees, np, plt, tree_depth):
    # Solution
    x_compare_1d = np.arange(0.0, 6.0 + 0.025, 0.05)
    x_features_compare_1d = x_compare_1d.reshape(-1, 1)
    y_output_compare_1d = f2_1D(x_compare_1d)

    y_pred_tree_compare_1d = model_1d.predict(x_features_compare_1d)
    mse_tree_1d = np.mean((y_pred_tree_compare_1d - y_output_compare_1d) ** 2)

    y_pred_forest_compare_1d = forest_1d.predict(x_features_compare_1d)
    mse_forest_1d = np.mean((y_pred_forest_compare_1d - y_output_compare_1d) ** 2)
...
@app.cell
def _(f2_2D, forest_2d, model_2d, n_trees, np, plt, tree_depth):
    # Solution
    x_compare_2d = np.arange(1.0, 6.0, 0.05)
    y_compare_2d = np.arange(1.0, 6.0, 0.05)
    xy_features_compare_2d = np.column_stack([x_compare_2d, y_compare_2d])
    z_output_compare_2d = f2_2D(x_compare_2d, y_compare_2d)

    z_pred_tree_compare_2d = model_2d.predict(xy_features_compare_2d)
    mse_tree_2d = np.mean((z_output_compare_2d - z_pred_tree_compare_2d)** 2)

    z_pred_forest_compare_2d = forest_2d.predict(xy_features_compare_2d)
    mse_forest_2d = np.mean((z_output_compare_2d - z_pred_forest_compare_2d)** 2)
...
# Solution
def huber_loss(delta_y, delta=0.5):
    if abs(delta_y) <= delta:
        return 0.5 * delta_y**2
    return delta * (abs(delta_y) - 0.5 * delta)
...
@app.cell
def _(delta, f2_1D, forest_1d, np, plt):
    # Solution
    x_loss = np.arange(0.0, 8.0, 0.01)
    x_features_loss = x_loss.reshape(-1, 1)
    y_output_loss = f2_1D(x_loss)
    y_pred_loss = forest_1d.predict(x_features_loss)

...
function fom_for_cut(sdf::DataFrame, bdf::DataFrame, var::AbstractString, cut::Real, fom::Function; op::Function = >, tf::Function=identity, nsig::Real=0, nbkg::Real=0)
	# for getting started, don't worry about any of the optional arguments

	# the function will return the function you passed for the figure of merit, applied to the number of signal and background events
	return fom(s, b)
end
...
begin  
	...
	# TODO: Loop through cut values
	for cut in cut_values	    
	    push!(ginis, fom_for_cut(...))
	    ...
	end
    # TODO: Get results for plotting
	best_index_gini = ...
	best_index_significances = ...
	best_cut_gini = ...
	best_cut_significances = ...
	print("Best cut gini: $best_cut_gini\n")
	print("Best cut best_cut_significances: $best_cut_significances")
end

# ╔═╡ 74665988-735a-4010-92d6-de480f3604e2
let 
	# TODO: Plot normalized ginis and significances as function of the cut values
	...
end
...
let
	...
	println("Signal efficiency gini: $(sig_effs[best_index_gini])")
	println("Background rejection gini: $(1 - bkg_effs[best_index_gini])")
	println("Signal efficiency significance: $(sig_effs[best_index_significances])")
	println("Background rejection significance: $(1 - bkg_effs[best_index_significances])")

	# TODO plot ROC curve
	
	# TODO: Add markers at best Gini and best significance
	scatter!(...)
	scatter!(...)	
end

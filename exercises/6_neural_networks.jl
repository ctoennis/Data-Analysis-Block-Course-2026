...
begin
	...
	cl_model = Chain(Dense(n_features, """number of output layers""", """activation"""), 
					 """more layers""" 
					)
	copts = Flux.setup("""choose your optimiser here""", cl_model)
...
let		
	for epoch in 1:1000
		t_loss, grads = Flux.withgradient(cl_model) do m
			...
			Flux.Losses."""choose your loss function"""(y_hat, ty_train)
		end
		Flux.update!(copts, cl_model, grads[1])
		v_loss = Flux.Losses."""choose your loss function"""(cl_model(tx_val), ty_val)
...
let	
	...
	plot(t_losses[1:end], label="Train Loss", lw=2)
	plot!(v_losses[1:end], label="Val Loss", lw=2, title="Loss Curve", xlabel="Epoch", ylabel="BCE")
end
...
let
	...
	plot!(normalize(hsv),label="sig val", mc=:cornflowerblue, lw=0, ms=4, seriestype=:scatter)
	plot!(normalize(hbt),label="bkg train", fillcolor=:red3, alpha=0.5, lw=0, seriestype=:stepbins, fillrange=0)
	plot!(normalize(hbv),label="bkg val", mc=:red3, lw=0, ms=4, seriestype=:scatter)
end
...
begin
	...
	n_training_samples = """How large should the sample be to see overtraining effects in ~100 epochs?"""
...
begin
	Random.seed!(12345)
	b_model = Chain("""Train an architecture of your choice""")
	bopts = Flux.setup("""Also choose an optimiser""", b_model)
	bt_losses = []; bv_losses = []
end
...
boot_t_losses, boot_v_losses = let
	...
	for i_boot in 1:n_boot
		...
		Random.seed!(12345)
		boot_model = Chain("""Copy the architecture that you tested above. Then try with regularisation."""
					)
		boot_opts = Flux.setup("""Copy the optimiser that you tested above""", boot_model)		
